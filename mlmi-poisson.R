mice.impute.2l.glmer.poisson <- function (y, ry, x, type, wy = NULL, intercept = TRUE, ...) 
{
  ...
  probM <- dpois(yobs, probM) #likelihood of each individual 
  ...
  for (jj in lev){
    ...
    logit <- as.vector(as.matrix(X[wy & x[, clust] == jj, , drop = FALSE]) %*% beta.star + 
                         as.matrix(Z[wy & x[, clust] == jj, , drop = FALSE]) %*% as.matrix(bi.star))
    prob <- exp(logit)
    y[wy & x[, clust] == jj] <- rpois(length(prob), prob)
  }
  return(y[wy])
}
